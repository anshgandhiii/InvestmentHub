...
export function VirtualStockDetail() {
  ...
  const chartData = {
    ...
    datasets: [{
      label: `${symbol} Virtual Price (₹)`,
      data: timestamps.slice(0, 20).map((ts) => parseFloat(stock["Time Series (60min)"][ts]["4. close"])),
      borderColor: "#6366F1", // Indigo to match VirtualMarket theme
  ...
  const chartOptions = {
    ...
    scales: {
      ...
      y: { grid: { color: "rgba(0, 0, 0, 0.05)" }, title: { display: true, text: "Virtual Price (₹)", font: { size: 14, weight: "bold" } } },
    },
    plugins: {
  ...
  const handleVirtualBuySell = async (action) => {
    if (!userId) {
    ...
    try {
      const response = await axios.post(
        ...
        {
          user_id: userId,
          virtual_asset_symbol: symbol,
          virtual_asset_type: "stock",
          virtual_price: stockInfo.price, // Send current price from stocks.json
          virtual_quantity: quantity,
          virtual_transaction_type: action,
        },
        { headers: { "User-Id": userId, "Content-Type": "application/json" } }
      );

      const successMsg = action === "buy"
        ? `Successfully bought ${quantity} virtual shares of ${symbol} at ₹${stockInfo.price}`
        : `Successfully sold ${quantity} virtual shares of ${symbol} at ₹${stockInfo.price}. Virtual Profit/Loss: ₹${response.data.profit_loss || "N/A"}`;
      setMessage(successMsg);
    } catch (error) {
      console.error(`Virtual ${action} error:`, error);
      const errorMsg = error.response?.data?.error || error.message;
      setMessage(`Failed to ${action} virtual ${symbol}: ${errorMsg}`);
    } finally {
  ...
  if (!stock) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white p-6 rounded-xl shadow-lg text-center text-gray-800">Virtual Stock not found.</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <header className="mb-8 text-center">
        ...
        <p className="text-gray-500 mt-1 text-lg">Virtual Stock Market Insights</p>
      </header>
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 flex items-center justify-between hover:shadow-lg transition-all duration-300">
          <div>
            ...
            <p className="text-sm text-gray-500">Current Virtual Price</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-all duration-300">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Virtual Stock Information</h3>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-all duration-300">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Latest Virtual Metrics</h3>
            <ul className="text-gray-600 space-y-3">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-all duration-300">
          <h3 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <FaChartLine className="text-indigo-600" /> Virtual Price Trend
          </h3>
          <div className="h-80">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-all duration-300">
          <h3 className="text-xl font-semibold text-gray-900 mb-4">Trade Virtual {symbol}</h3>
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            <button
              onClick={() => handleVirtualBuySell("buy")}
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <FaWallet /> Buy Virtual
                </>
              )}
            </button>
            <button
              onClick={() => handleVirtualBuySell("sell")}
              disabled={isSubmitting}
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <FaWallet /> Sell Virtual
                </>
              )}
            </button>
          </div>
          <p className="mt-3 text-sm text-gray-500">
            Total Virtual Cost/Value: <span className="font-medium text-gray-800">₹{(stockInfo.price * quantity).toFixed(2)}</span>
          </p>
          {message && (
        ...
        <div className="text-center">
          <Link
            to="/virtualmarket" // Adjust route to match your VirtualMarket path
